...
- **🔍 多维度筛选**：支持按掌握状态、时间范围、知识点标签、年级学期、试卷等级、错题类型（**多选**）等多种条件筛选错题。
- **📅 智能复习**：到期错题自动提醒，支持**顺序 / 倒序 / 乱序 / 按知识点**四种排序，一键打印复习卷（卷面跟随排序，变式题隐藏难度）。
...
基于艾宾浩斯遗忘曲线自动安排复习计划，到期错题在错题本顶部横幅提醒，可逐题拍照 AI 批改或手动录入对错。

### 复习排序（四档）

切换入口：错题本页「今日待复习」横幅展开后的**排序**按钮。选择保存在浏览器中（按错题本分开记忆），下次进入保持上次的选择，可随时手动调整。

| 排序 | 效果 | 适用场景 |
| :--- | :--- | :--- |
| **顺序** | 掌握度低优先、到期早优先 | 默认，先攻最薄弱的题 |
| **倒序** | 同上反向 | 从较熟的题热身进入状态 |
| **乱序** | 随机打散（同一批题顺序固定，刷新不变） | 后期检验整体掌握，避免按位置记忆 |
| **知识点** | 相同知识点的题聚在一起显示 | 前期按知识点逐块针对性突破 |

> 每日数量限额圈定的始终是「最薄弱 + 最早到期」的一批题，排序只改变呈现顺序。**「打印复习卷」自动跟随当前排序**。

### 打印复习卷

- 卷面题目顺序与所选排序一致（选乱序则卷面同序，防止孩子背题号位置）。
- 勾选「举一反三」附加变式题时，卷面只显示「变式1、变式2…」序号，**不显示难度**，避免孩子做题前受难度暗示；难度勾选区仅家长可见。

### 每日复习数量

设置 → 通用 → 每日复习数量：可选 5 ~ 500 题或不限（选「不限」时接口侧仍以 500 兜底，防止一次拉爆）。

### 错因多选筛选

错题列表的「全部错因」下拉支持**多选**：可同时勾选「概念不清」+「公式记错」等，把没理解的错因一次性列出来查看总量；也可勾选「未分类」一并纳入。筛选结果可一键导出打印。
...
docker run -d --name wrong-notebook \
  ...
  ghcr.io/mingyanplus/wrong-notebook
```
...
1.  **下载配置文件**：
    ```bash
    curl -o docker-compose.yml https://raw.githubusercontent.com/mingyanplus/wrong-notebook/refs/heads/main/docker-compose.yml
    ```
2.  **启动服务**：
    ```bash
    docker-compose up -d
...
```bash
git clone https://github.com/mingyanplus/wrong-notebook.git
cd wrong-notebook
```
...
设置 → 通用 → **AI 分析超时（秒）**：可选 120 ~ 1800（30 分钟）。该值同时控制**前端等待时长**与**后端 AI 请求超时**——使用思考模型（如 GLM 深度思考）偶发慢响应超时的时候，把这里调大即可；需要运维强制覆盖时用 `AI_HTTP_TIMEOUT` 环境变量。
